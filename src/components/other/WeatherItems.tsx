...
const TempItem = ({ children }: TempItemPros): React.ReactNode => {
  return (
    <div
      className={`flex items-center justify-between border border-white/50 p-2 md:text-xl`}
    >
      {children}
    </div>
  );
      ...
      {entries.map(([key, value], entryIndex: number) => {
        if ("hourly".includes(key)) {
          ...
          return (
            <TempItem key={entryIndex}>
              <h3>{(dataKeys as any)[key]}：</h3>
              <span className="w-30 text-center text-sm text-red-400 md:w-60">
                （未必精準）
                <br /> 負值為 °S
              </span>
          ...
          return (
            <TempItem key={entryIndex}>
              ...
              <p className="w-30 text-end">
                {value > 0
                  ? `${String(value.toFixed(2))} °E`
                  : `${String(value.toFixed(2))} °W`}
              </p>
            </TempItem>
          );
...
export const Hourly = ({ data, isOpen, setIsOpen }: HourlyProps) => {
  ...
  return (
    <div
      className={`hourly absolute top-0 left-0 z-[10] flex h-[100vh] w-[100vw] flex-col gap-4 overflow-y-auto bg-black/80 p-2 pb-4 backdrop-blur-sm transition duration-1000 md:h-full md:max-w-1/2 md:p-8 lg:w-150 ${isOpen ? "translate-x-0 opacity-100" : "-translate-x-150 opacity-0"} `}
    >
      <div className="flex w-full justify-end px-0">
        <Button
          label="❎"
      ...
      <div className="flex items-center justify-center gap-4 border p-4">
        ...
        <div className="h-full">
          {hourly_units &&
            Object.entries(hourly_units).map(([key, value], index) => {
              if (key === "time") return;
              return (
                <p key={index}>
                  {`2 公尺高度的氣溫`} ： {String(value)}
                </p>
              );
            })}
        </div>
      ...
      <div className="border">
        <table className="w-full text-center">
          <thead>
            <tr className="py-">
              {hourly &&
                Object.keys(hourly).map((h, index) => (
            ...
            {timeList &&
              timeList.map((time, index) => {
                return (
                  <tr
                    key={index}
                    className="odd:hover:bg-slate-700 even:hover:bg-slate-500"
                  >
                    <td className="border py-2">{time}</td>
                    <td className="border py-2">{tempList[index]}°C</td>
                  </tr>
                );
              })}
          </tbody>
