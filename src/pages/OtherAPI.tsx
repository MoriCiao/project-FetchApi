...
import {
  ...
  updateData,
  setLoading,
} from "../features/otherApi/otherSlice";
...
const OtherAPI = () => {
  const { currentURL, link, statusApi } = useSelector(
  ...
  return (
    <Fade triggerOnce={true} className={`h-full w-full`}>
      <div className="relative h-full w-full px-4 pb-0 text-white md:px-12">
        <div className="other-header flex h-auto w-auto flex-col items-center justify-center gap-4 md:h-[10%] md:flex-row">
          {/* @ts-ignore */}
          <Button
            label="Reset"
            onClick={handleReset}
            otherStyle="rounded-md md:w-auto w-full max-w-80 py-2 bg-white/20 hover:bg-white/50 hover:text-black transition duration-200"
          />
          <FetchStatus type={status.type} msg={status.msg} />
          <Select />
        </div>
        <div className="other-main h-full w-full overflow-auto md:p-8 lg:h-[90%]">
          {status.type === "success" && currentURL === link.bike_url && (
            <YouBike />
          )}
          {status.type === "success" && currentURL === link.weather_url && (
            <Weather />
          )}
        </div>
